...
import org.apache.hyracks.control.cc.job.JobManagerNoLineSkip;
...
public class Wisconsin_V1 implements IJobQueue {
    ...
    public Wisconsin_V1(IJobManager jobManager, IJobCapacityController jobCapacityController) {
    ...
    private MPLQueue getQueue(JobSpecification.JobSizeTag sizeTag) {
        if (sizeTag == JobSpecification.JobSizeTag.SMALL || sizeTag == JobSpecification.JobSizeTag.ZERO
                || sizeTag == JobSpecification.JobSizeTag.ZERO_SHORT
                || sizeTag == JobSpecification.JobSizeTag.ZERO_LONG) {
    ...
    private void printAllStats() {
        StringBuilder sb = new StringBuilder();
        //activeRunMap
        LOGGER.warn("ActiveRunMap:" + ((JobManagerNoLineSkip) jobManager).printElementsInActiveRunMap());
