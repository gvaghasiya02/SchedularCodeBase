...
public class GraefesQueue implements IJobQueue {
    ...
    public GraefesQueue(IJobManager jobManager, IJobCapacityController jobCapacityController) {
        ...
        double[] candidateExecTimes =
                new double[] { 55.5, 27.33, 41.4, 58.55, 76.27, 124.3, 160.46, 215.23, 295.49, 337.32 };
    ...
    private MPLQueue getQueue(double ratio) {
        if (ratio <= 0.05) {
            return queues.get(0);
        } else if (ratio <= 0.10) {
            return queues.get(1);
    ...
    public void add(JobRun run) throws HyracksException {
        ...
        MPLQueue queue = getQueue(jobCapacityController.getMemoryRatio(run.getJobSpecification()));
