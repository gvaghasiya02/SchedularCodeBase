...
public class PhysicalOptimizationConfig {
    ...
    public int getMaxFramesForJoinLeftInput(long userSpecifiedBuildSize) {
        int frameSize = getFrameSize();
        if (userSpecifiedBuildSize > 0)
            setMaxFramesForJoinLeftInput((int) (userSpecifiedBuildSize * MB / frameSize));
    ...
    public int getMaxFramesForJoin(long userSpecifiedMemory) {
        if (userSpecifiedMemory <= 0)
            return getMaxFramesForJoin();
        int frameSize = getFrameSize();
        return (int) (userSpecifiedMemory * 1024 / frameSize);
    }
